...
#Import MasterFile
Master <- read_excel("2024_06_27_MasterFile_Analysis.xlsx")
# Remove exclusion
Master <- Master %>%
...
Master.v2 <- Master %>%
  ...
  dplyr::select(c("REDCAP_SEQ_NO","redcap_event_name","T2_high_low","ma_depj1","ma_depj2","ma_depj3","ma_depj4","ma_depj5","ma_depj6","ma_moy_j1","ma_moy_j2","ma_moy_j3","ma_moy_j4","ma_moy_j5","ma_moy_j6"))
...
summary(fit)
mat[26,] <-  c("Gina Step - 4",round(Confint(fit)[2,1],1),round(Confint(fit)[2,2],1),round(Confint(fit)[2,3],5),round(summary(fit)$coefficient[2,4],4))
mat[27,] <-  c("Gina Step - 5",round(Confint(fit)[3,1],1),round(Confint(fit)[3,2],1),round(Confint(fit)[3,3],5),round(summary(fit)$coefficient[3,4],4))

fit <- lm( V2_V1_Pre_FEV1_L ~ BloodEOS_Abso_log10*FeNO_log10, data= Master.patient ) 
summary(fit)
mat[28,] <-  c("Interaction",round(Confint(fit)[4,1],1),round(Confint(fit)[4,2],1),round(Confint(fit)[4,3],5),round(summary(fit)$coefficient[4,4],4))
...
forplo(mat[,2:4]  , row.labels = c("Log10 BEC","Log10 FeNO","Log10 BEC : Log10 FeNO interaction*","Log10 spEos","Log10 Sputum TCC","T2 High","Previously T2 high","Log10 total IgE ","FEV1% (per 10% decrease)","FEV1/FVC","Delta FEV1% (per 10% decrease)","Reversibility (per 10% decrease)","Age","Sex - Male","BMI","Smoking status - Ex-smoker","ACQ-5 (per 0.5 increase)","Severe asthma attacks (past 12 months)","MART","Adherence (per 10% decrease)","Any IgE sensitization","Nasal polyps","Charlson Comorbidity Index score","Viral associated exacerbation","Antibiotic prescribed","Bacterial infection on sputum culture","GINA step 4 (vs steps 1-3)","GINA step 5 (vs steps 1-3)"),add.columns= as.data.frame(cbind(rep(NA,28),format(round(mat$P.value,3),nsmall = 3 ))),
...
mat[26,] <-  c("Gina Step - 4",round(exp(Confint(fit)[2,1]),1),round(exp(Confint(fit)[2,2]),1),round(exp(Confint(fit)[2,3]),5),round(summary(fit)$coefficient[2,4],4))
mat[27,] <-  c("Gina Step - 5",round(exp(Confint(fit)[3,1]),1),round(exp(Confint(fit)[3,2]),1),round(exp(Confint(fit)[3,3]),5),round(summary(fit)$coefficient[3,4],4))

fit <- glm( V2_V1_Pre_FEV1_L_cat0.1 ~ BloodEOS_Abso_log10*FeNO_log10, family = binomial(link="logit"), data= Master.patient ) 
summary(fit)
mat[28,] <-  c("Interaction",round(exp(Confint(fit)[4,1]),1),round(exp(Confint(fit)[4,2]),1),round(exp(Confint(fit)[4,3]),5),round(summary(fit)$coefficient[4,4],4))
...
table1( ~ OLOEOS_Abso + OLOEOS_PORC + OLOEOS_Type + OLOEOS_repnum  | T2_high_low, data=Master,
       render.continuous=c(.="Mean (SD)", .="Median [Q1 - Q3]"), topclass="Rtable1-grid Rtable1-shade Rtable1-times")
...
icc(
  Master[,c("BloodEOS_Abso","OLOEOS_Abso")], model = "twoway", 
